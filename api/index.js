...
//     app.locals.squareStatesCollection = squareStatesCollection;
...
//   await req.app.locals.usersCollection.insertOne(user); // Insert the user into the collection
...
//   const { email, password } = req.body;
//   const user = await req.app.locals.usersCollection.findOne({ email }); // Find user by email
...
//   const state = await req.app.locals.squareStatesCollection.findOne({}); // Find the current state
...
//   let state = await req.app.locals.squareStatesCollection.findOne({}); // Find the current state
//   if (!state) {
//     state = new SquareState(squares); // Create a new state if none exists
//     await req.app.locals.squareStatesCollection.insertOne(state); // Insert the new state
//   } else {
//     await req.app.locals.squareStatesCollection.updateOne(
//       { _id: state._id },
//       { $set: { squares: squares } }
